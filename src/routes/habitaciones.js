...
router.get('/', async (req, res) => {
    try {
        const datos = await roomsController.getRooms();
...
// GET /rooms/disponibles?check_in=&check_out=&guests= → buscar habitaciones disponibles
router.get('/disponibles', async (req, res) => {
    try {
        ...
        const datos = await roomsController.getAvailableRooms(check_in, check_out, guests);
...
router.get('/:id', async (req, res) => {
    try {
        const { id } = req.params;
        const datos = await roomsController.getRoomById(id);
...
router.post('/', async (req, res) => {
    try {
        ...
        const result = await roomsController.insertRoom(req.body);
...
router.put('/:id', async (req, res) => {
    try {
        ...
        const result = await roomsController.updateRoom({ ...req.body, id });
...
router.delete('/:id', async (req, res) => {
    try {
        const { id } = req.params;
        const result = await roomsController.deleteRoom(id);
        if (result.affectedRows === 0) return error(res, 'Habitación no encontrada', 404);
